...
from chudvis.gaze.features import HeadNormalizedGazeExtractor
from chudvis.perception.models import ensure_model
...
class MediaPipeTracker:
    ...
    @staticmethod
    def _physical_handedness(label: str) -> str:
        """Convert the detector's mirrored-view label to the user's physical hand."""
    ...
    def process(self, bgr_frame: Any, timestamp: float | None = None) -> PerceptionResult:
        ...
        for index, landmarks in enumerate(hand_result.hand_landmarks or ()):
            ...
            if index < len(handedness_results) and handedness_results[index]:
                ...
                handedness = self._physical_handedness(category.category_name or "unknown")
