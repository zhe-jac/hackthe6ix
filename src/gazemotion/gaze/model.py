...
from gazemotion.gaze.features import FEATURE_COUNT as HEAD_NORMALIZED_FEATURE_COUNT
    ...
    def __post_init__(self) -> None:
        if self.profile_version != PROFILE_VERSION or self.feature_backend != FEATURE_BACKEND:
            raise ValueError(
                ...
                "Run `gazemotion calibrate` to create a head-normalized profile."
            )
    ...
    def load(cls, path: Path | None = None) -> CalibrationProfile:
        ...
        if (
            ...
            raise ValueError(
                ...
                "Run `gazemotion calibrate` again."
            )
        return cls(**data)
